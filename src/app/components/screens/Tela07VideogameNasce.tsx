    ...
    <StoryScreenFrame
      ...
      right={
        <div className="flex h-full w-full flex-col items-center justify-center gap-6">
          <div className="flex items-center gap-12 rounded-3xl border-[6px] border-black bg-white px-12 py-6 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)]">
            <div className="text-center">
              <p className="mb-2 text-[24px] font-bold text-[var(--sesc-gray-stone)]">VOCÊ</p>
              <p className="text-[72px] font-black text-[var(--sesc-blue-dark)]">{score.player}</p>
            </div>
            <div className="text-[48px] font-black">×</div>
            <div className="text-center">
              <p className="mb-2 text-[24px] font-bold text-[var(--sesc-gray-stone)]">CPU</p>
              <p className="text-[72px] font-black text-[var(--sesc-pink)]">{score.cpu}</p>
            </div>
          </div>

          <div className="rounded-2xl border-[4px] border-black bg-[var(--sesc-yellow-flower)] px-8 py-4">
            <p className="text-[24px] font-bold uppercase text-black">Arraste a raquete para jogar</p>
          </div>

          <div
            ref={gameRef}
            className="relative cursor-none touch-none border-[8px] border-white bg-black shadow-[12px_12px_0px_0px_rgba(0,0,0,1)]"
          ...
          >
            <div
              className="absolute bottom-0 left-1/2 top-0 w-[4px] bg-white opacity-30"
              style={{
                backgroundImage:
            ...
            <div
              ...
              style={{ left: '0px', top: `${paddleY}px`, width: `${PADDLE_WIDTH}px`, height: `${PADDLE_HEIGHT}px` }}
            ></div>
            <div
              className="absolute bg-white"
              style={{ right: '0px', top: `${cpuPaddleY}px`, width: `${PADDLE_WIDTH}px`, height: `${PADDLE_HEIGHT}px` }}
            ></div>
            <div
              className="absolute rounded-sm bg-white"
              style={{ left: `${ballPos.x}px`, top: `${ballPos.y}px`, width: `${BALL_SIZE}px`, height: `${BALL_SIZE}px` }}
            ></div>
          </div>
        </div>
      }
